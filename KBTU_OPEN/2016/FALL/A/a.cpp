...
int getNext(node * A, node * B, int tl, int tr, int k)
{
	if (tl == tr) 
	{
	...
	else
	{
		...
		if (cur >= k) return getNext(A -> l, B -> l, tl, mid, k);
		return getNext(A -> r, B -> r, mid + 1, tr, k - cur);
	}
}
...
bool ok(int x, int cur)
{
	...
	while (cnt < 4)
	{
		...
		int g = getNext(& t[top], & t[bot + 1], 0, n - 1, k + 1);
		if (g == -1) break;
